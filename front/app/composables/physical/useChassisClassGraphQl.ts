...
import CHASSIS_CLASSESFILE from '@/graphql/physical/chassis_class/chassisClassesFile.query.gql'
...
export const useChassisClassGraphQl = () => {
    ...
    const loadQuery = async (nameOrId?: string, onlyChassisClass?: boolean) => {
        try { 
            await load( { nameOrId: nameOrId, onlyChassisClass: onlyChassisClass || undefined})
        } catch (e) {
    ...
    const { result: fileResult, load: loadFileQuery, onError: onLoadFileError } = useLazyQuery<ChassisClassesFileData>(CHASSIS_CLASSESFILE,
