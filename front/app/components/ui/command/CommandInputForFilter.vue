...
// Ajout pour pouvoir relancer la recherche une fois 
// l'ajout dynamique d'un élément
const model  = defineModel()
filterState.search = model.value as string

watch(model, (value) => {
  if (filterState.search !== value) {
    filterState.search = value as string
  }
})

watch(filterState, (value) => {
  if (model.value !== filterState.search) {
